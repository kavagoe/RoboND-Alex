...
def create_output_images(Rover):
      ...
      if np.max(Rover.worldmap[:,:,0]) > 0:
            ...
            obstacle = Rover.worldmap[:,:,0] * (255 / np.mean(Rover.worldmap[obs_pix, 1]))
      else:
            obstacle = Rover.worldmap[:,:,0]
      
      likely_nav = navigable >= obstacle
      obstacle[likely_nav] = 0
      #plotmap = np.zeros_like(Rover.vision_image)
      plotmap = np.zeros_like(Rover.worldmap)
      plotmap[:, :, 0] = obstacle
      plotmap[:, :, 2] = navigable
      #plotmap[:, :, 2] = Rover.vision_image[:,:,2]
      ...
      #map_add = cv2.addWeighted(plotmap, 1, Rover.vision_image, 0.5, 0)
      ...
      vision=Rover.vision_image
      pil_img = Image.fromarray(vision.astype(np.uint8))
